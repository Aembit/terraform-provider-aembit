...
resource "aembit_trust_provider" "oidcidtoken_aembittenantoidctoken" {
	name = "TF Acceptance OIDC ID Token AembitTenantOidcToken"
	is_active = true
	oidc_id_token = {
		issuer = "issuer"
		subject = "subject"
...
resource "aembit_trust_provider" "oidcidtoken_customclaims" {
	name = "TF Acceptance OIDC ID Token with Custom Claims"
	is_active = true
	oidc_id_token = {
		custom_claims = [
    	  {
